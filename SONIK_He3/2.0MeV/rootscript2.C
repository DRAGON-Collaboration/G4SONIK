...
void rootscript2()
{
	...
	for (int p=0;p<5;p++)
	{
		cout << "Z    SolidAngle  Conversion  CrossSection  Events" << endl;     
		for (int q=0;q<11;q++)
		{
			TString str = "/local/astro/scat/SONIK_He3/2.0MeV/";
			str.Append(col[p]);
			str.Append(sls);
			/*
			...
			*/
			if (p == 0) {str = "/local/astro/scat/SONIK_He3/2.0MeV/1mm_10k_pnA/";}
			if (p == 1) {str = "/local/astro/scat/SONIK_He3/2.0MeV/2mm_10k_pnA/";}
			if (p == 2) {str = "/local/astro/scat/SONIK_He3/2.0MeV/3mm_10k_pnA/";}
			if (p == 3) {str = "/local/astro/scat/SONIK_He3/2.0MeV/5mm_10k_pnA/";}
			if (p == 4) {str = "/local/astro/scat/SONIK_He3/2.0MeV/7mm_10k_pnA/";}

...
for i in "${col[@]}";do
	...
	for j in "${deg[@]}";do
		...
		mkdir "$j"deg
  		cp "/local/astro/scat/SONIK_He3/1.0MeV/3He4He.dat" "$j""deg/3He4He.dat"
		cp "/local/astro/scat/SONIK_He3/1.0MeV/config.dat" "$j""deg/config.dat"
		...
		while read line;do
			...
			((k++))
		done < "/local/astro/scat/SONIK_He3/1.0MeV/SONIK.dat" > "$j""deg/SONIK.dat"

...
for i in "${col[@]}";do
	...
	for j in "${deg[@]}";do
		...
		mkdir "$j"deg
  		cp "/local/astro/scat/SONIK_He3/1.5MeV/3He4He.dat" "$j""deg/3He4He.dat"
		cp "/local/astro/scat/SONIK_He3/1.5MeV/config.dat" "$j""deg/config.dat"
		...
		while read line;do
			...
			((k++))
		done < "/local/astro/scat/SONIK_He3/1.5MeV/SONIK.dat" > "$j""deg/SONIK.dat"
